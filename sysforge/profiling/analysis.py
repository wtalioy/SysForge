...
from ..integrations.llm import LLMError, chat_cuda, chat_json
...
class AnalysisRunner:
    ...
    def _compile_reference(self, source: str):
        source_path = self.workspace.write_source("gemm_reference", 1, source)
        binary_path = self.workspace.binary_path("gemm_reference", 1)
        ...
        source_path = self.workspace.write_source("gemm_reference", 2, fixed_source)
        binary_path = self.workspace.binary_path("gemm_reference", 2)
        compile_result = compiler.compile_cuda(
            source_path, binary_path, arch=self.config.nvcc_arch, timeout_s=self.config.compile_timeout_s
        )
        return compile_result, binary_path

    def _format_ncu_error(self, ncu_result) -> str:
    ...
    def run(self, metrics: list[str]) -> AnalysisOutcome:
        output = AnalysisOutcome(metrics_requested=list(metrics))
        try:
            source = chat_cuda(load_prompt("generate_gemm.txt"), system=SYSTEM_PROMPT, temperature=0.2)
        except LLMError as exc:
            output.error = f"LLM generate_gemm failed: {exc}"
            return output

        try:
            compile_result, binary_path = self._compile_reference(source)
        except LLMError as exc:
        ...
        run_result = executor.run_binary(binary_path, timeout_s=self.config.run_timeout_s)
        if not run_result.ok:
            output.error = f"gemm run failed rc={run_result.rc}: {run_result.stderr[-500:]}"
            return output

        ncu_result = ncu.profile(binary_path, metrics, timeout_s=self.config.ncu_timeout_s)
        if not ncu_result.ok:
            output.error = self._format_ncu_error(ncu_result)
