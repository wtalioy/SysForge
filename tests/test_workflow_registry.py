...
def test_optimize_lora_workflow_emits_progress_logs(monkeypatch, tmp_path, capsys):
    monkeypatch.setenv("TARGET_DIR", str(tmp_path / "target"))
    monkeypatch.setenv("WORKSPACE_DIR", str(tmp_path / "workspace"))
    monkeypatch.setenv("OPTIMIZE_LORA_TEST_MODE", "1")
    monkeypatch.setenv("OPTIMIZE_LORA_MAX_LLM_ROUNDS", "1")
    ...
    workflow = build_registry()["optimize-lora"]
    workflow.run(build_runtime_context(load_config()))
    stdout = capsys.readouterr().out

    assert "[sysforge][optimize-lora] starting optimize-lora" in stdout
    assert "round 1 started" in stdout
    assert "compiling llm_family-r1-v1" in stdout
    assert "tier1 complete for llm_family-r1-v1" in stdout
    assert "final winner:" in stdout
